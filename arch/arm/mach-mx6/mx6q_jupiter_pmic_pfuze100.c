...
static int pfuze100_init(struct mc_pfuze *pfuze)
{
	...
	if (value == 0x10) {
		...
		for (i = 0; i < 7; i++) {
			...
			ret = pfuze_reg_write(pfuze, PFUZE100_SW1AMODE + (i * 7),
					value);
			if (ret)
				goto err;
		}
...
static struct pfuze_regulator_init_data mx6q_jupiter_pfuze100_regulators[] = {
...
static struct pfuze_platform_data pfuze100_plat = {
	...
	.num_regulators = ARRAY_SIZE(mx6q_jupiter_pfuze100_regulators),
	.regulators = mx6q_jupiter_pfuze100_regulators,
...
int __init mx6q_jupiter_init_pfuze100(u32 int_gpio)
{
	if (int_gpio)
